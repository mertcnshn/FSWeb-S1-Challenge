...
function indekseGoreFenomen(arr, ind) {
  return `${ind}. indekste bulunan fenomen: ${arr[ind].profile}`;
}
...
function profilListesi(arrayFenomen) {
  const yeniDizi = [];

  for (let i = 0; i < arrayFenomen.length; i++) {
    yeniDizi.push(arrayFenomen[i].profile);
  }
  return yeniDizi;
}

//const yeniDizi =[...arrayFenomen]; Kopyalamanın 2. yolu
...
function fenomenSil(arrayFenomen, ind) {
  //const yeniDizi = [...arrayFenomen]; 2.yol bu.

  const yeniDizi = [];

  for (let i = 0; i < arrayFenomen.length; i++) {
    yeniDizi.push(arrayFenomen[i]);
  }
  yeniDizi.splice(ind, 1);
  return yeniDizi;
}
...
function fenomenEkle(fArray, fNumber, fProfile, fFollowers, fPost, fPlatform) {
  const newDizi = [...fArray];
  // const newDizi =fArray.slice(); Kopyalamanın 3. yolu
...
function enFenomenler(arrayFenomen) {
  const yeniDizi = [];

  for (let i = 0; i < arrayFenomen.length; i++) {
    if (arrayFenomen[i].followers > 100000000) {
      yeniDizi.push(arrayFenomen[i].profile);
    }
  }
  return yeniDizi;
...
function fenomenGonderimSayisi(fenoArray, fenoProfile) {
  for (let i = 0; i < fenoArray.length; i++) {
    if (fenoArray[i].profile === fenoProfile) {
      return fenoArray[i].posts;
    }
  }
}
...
function platformaGoreCokGonderiYapanFenomen(arrayFenomen, pPlatform) {
  let enCokGonderiProfile = "";
  let enCokGonderiSayisi = 0;

  for (let i = 0; i < arrayFenomen.length; i++) {
    if (
      arrayFenomen[i].posts !== "NA" &&
      arrayFenomen[i].posts > enCokGonderiSayisi &&
      arrayFenomen[i].platform === pPlatform
    ) {
      enCokGonderiProfile = arrayFenomen[i].profile;
      enCokGonderiSayisi = arrayFenomen[i].posts;
    }
  }
  return enCokGonderiProfile;
}
console.log(
  "Görev 9:",
